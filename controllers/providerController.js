...
export const createProviderController = async (req, res) => {
  try { 
    ...
    const existingCategory = await providerModel.findOne({nombre: name });
    if (existingCategory) {
      return res.status(200).send({
        success: false,
        message: "Este proveedor existe",
      });
    }
    ...
    const category = await new providerModel({
      nombre: name,
      codigo: slugify(name),
...
export const updateProviderController = async (req, res) => {
  try {
    ...
    let category = await providerModel.findById(req.params.id);
...
export const providerControlller = async (req, res) => {
  try {
    const category = await providerModel
    .find({})
    res.status(200).send({
      success: true,
...
// UNA SOLA CATEGORÍA
export const singleProviderController = async (req, res) => {
  try {
    const category = await providerModel.findOne({ _id: req.params.codigo })
    .select("-imagen"); 
    res.status(200).send({
      success: true,
...
export const providerPhotoController = async (req, res) => {
  try {
    
    const category = await providerModel.findById(req.params.pid).select("imagen");
